...
f=figure(1); set(f,'outerposition',[276   225   730   799],'position',[286   257   710   664]);
	...
	subplot(2,2,1)
		...
		plot([-7 5],[ 0  0],'k'); hold on
	       	plot(reshape(squeeze(t_GCM_11_GRL(:,:,1)),Nx*Ny,1),reshape(squeeze(r_GCM_11_GRL(:,:,1)),Nx*Ny,1),style{1},'markersize',tip); hold on
 		plot(reshape(squeeze(t_GCM_11_MIT(:,:,1)),Nx*Ny,1),reshape(squeeze(r_GCM_11_MIT(:,:,1)),Nx*Ny,1),style{2},'markersize',tip); hold on
	...
	subplot(2,2,3)
		plot([ 0 0],[-5 13],'k'); hold on
		plot([-7 5],[ 0  0],'k'); hold on
		plot(reshape(squeeze(t_GCM_11_GRL(:,:,3)),Nx*Ny,1),reshape(squeeze(r_GCM_11_GRL(:,:,3)),Nx*Ny,1),style{1},'markersize',tip); hold on
		plot(reshape(squeeze(t_GCM_11_MIT(:,:,3)),Nx*Ny,1),reshape(squeeze(r_GCM_11_MIT(:,:,3)),Nx*Ny,1),style{2},'markersize',tip); hold on
			xlabel('dT2m (deg C)'); ylabel('dR (mm/d)')
			xlim([-7 5]); ylim([-5 13])
			title('12.5km JJA')
			for Q=[1:4];
...
for i=1:cNx; 
	...
	subplot(2,2,2)
		plot([ 0 0],[-5 13],'k'); hold on
		plot([-7 5],[ 0  0],'k'); hold on
		plot(reshape(squeeze(t_GCM_44_GRL(:,:,1)),cNx*cNy,1),reshape(squeeze(r_GCM_44_GRL(:,:,1)),cNx*cNy,1),style{1},'markersize',tip); hold on
		plot(reshape(squeeze(t_GCM_44_MIT(:,:,1)),cNx*cNy,1),reshape(squeeze(r_GCM_44_MIT(:,:,1)),cNx*cNy,1),style{2},'markersize',tip); hold on
			xlabel('dT2m (deg C)'); ylabel('dR (mm/d)')
			xlim([-7 5]); ylim([-5 13])
			title('50km DJF')
			for Q=[1:4];
	...
	subplot(2,2,4)
		plot(reshape(squeeze(t_GCM_44_GRL(:,:,3)),cNx*cNy,1),reshape(squeeze(r_GCM_44_GRL(:,:,3)),cNx*cNy,1),style{1},'markersize',tip); hold on
		plot(reshape(squeeze(t_GCM_44_MIT(:,:,3)),cNx*cNy,1),reshape(squeeze(r_GCM_44_MIT(:,:,3)),cNx*cNy,1),style{2},'markersize',tip); hold on
		plot([ 0 0],[-5 13],'k'); hold on
		plot([-7 5],[ 0  0],'k'); hold on
			xlabel('dT2m (deg C)'); ylabel('dR (mm/d)')
			xlim([-7 5]); ylim([-5 13])
	...
	FILENAME_OUTPUT=['dT2m_vs_dR_',AREAtxt{AREA},'_commonLimits_GCMonly.png'];
